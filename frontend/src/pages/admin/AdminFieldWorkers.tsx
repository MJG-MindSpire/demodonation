...
export default function AdminFieldWorkers() {
  ...
  const fieldWorkersQuery = useQuery({
    queryKey: ["admin", "field-workers"],
    queryFn: async () => {
      const res = await apiRequest<{ users: FieldWorkerRow[] }>("/api/admin/field-workers");
      return res.users;
    },
  });
  ...
  const verifyMutation = useMutation({
    mutationFn: async (id: string) => {
      return apiRequest<{ user: FieldWorkerRow }>(`/api/admin/field-workers/${id}/verify`, {
        method: "POST",
        body: JSON.stringify({}),
    ...
    onSuccess: async (_data, id) => {
      await queryClient.invalidateQueries({ queryKey: ["admin", "field-workers"] });
      toast({ title: "Verified", description: "Field worker has been verified." });
  ...
  const statusMutation = useMutation({
    mutationFn: async ({ id, isActive }: { id: string; isActive: boolean }) => {
      return apiJson<{ user: FieldWorkerRow }>(`/api/admin/field-workers/${id}/status`, { isActive }, { method: "PATCH" });
    },
  ...
  const filtered = useMemo(() => {
    const list = fieldWorkersQuery.data ?? [];
  ...
  }, [fieldWorkersQuery.data, filter]);

  const totalCount = fieldWorkersQuery.data?.length ?? 0;
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="relative flex flex-col gap-4 p-6 md:flex-row md:items-center md:justify-between">
          <div className="space-y-2">
            <div className="inline-flex w-fit items-center gap-2 rounded-full border bg-white/60 px-3 py-1 text-xs font-medium text-muted-foreground">
              ...
              Field Workers ({totalCount})
            </div>
            <h1 className="text-2xl font-bold tracking-tight md:text-3xl">Field Workers</h1>
            <p className="text-muted-foreground">Review registered field workers and their profile details</p>
          </div>
      ...
      {fieldWorkersQuery.isLoading && (
        <Card className="border-white/30 bg-white/60 shadow-[var(--shadow-card)] backdrop-blur-md">
          <CardContent className="p-6">
            <p className="text-sm text-muted-foreground">Loading field workers...</p>
          </CardContent>
        </Card>
      )}

      {fieldWorkersQuery.isError && (
        <Card className="border-white/30 bg-white/60 shadow-[var(--shadow-card)] backdrop-blur-md">
          <CardContent className="p-6">
            <p className="text-sm text-destructive">Failed to load field workers.</p>
          </CardContent>
        </Card>
      )}
            ...
            <CardContent className="relative space-y-3">
              ...
              >
                ...
                <DialogContent className="max-w-xl">
                  <DialogHeader>
                    <DialogTitle>Field Worker Details</DialogTitle>
                  </DialogHeader>
      ...
      {!fieldWorkersQuery.isLoading && !fieldWorkersQuery.isError && filtered.length === 0 && (
        <Card className="border-white/30 bg-white/60 shadow-[var(--shadow-card)] backdrop-blur-md">
          <CardContent className="p-6">
            <p className="text-sm text-muted-foreground">No field workers found.</p>
          </CardContent>
        </Card>
      )}
    </div>
